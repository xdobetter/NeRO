...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        out_dim = 0
        if self.kwargs['include_input']: # 是否要包含输入的维度，默认包含
        ...
        self.embed_fns = embed_fns # postion encoding的函数
        self.out_dim = out_dim

    def embed(self, inputs): # 返回position encoding的结果
...
def get_embedder(multires, input_dims=3):
    embed_kwargs = {
        ...
        'max_freq_log2': multires - 1, # 5
        'num_freqs': multires, # 6
        'log_sampling': True,
...
class SDFNetwork(nn.Module):
    def __init__(self,
                 ...
                 geometric_init=True,
                 weight_norm=True,
                 inside_outside=False, # 室内场景是True，室外场景是False
                 sdf_activation='none',
        ...
        if layer_activation == 'softplus': # 激活函数选择
    ...
    def sdf(self, x):
        return self.forward(x)[..., :1] # 拿feature
...
class NeRFNetwork(nn.Module):
    def __init__(self,
        ...
        if multires > 0: # 输入4维的encoding
            embed_fn, input_ch = get_embedder(multires, input_dims=d_in)
            self.embed_fn = embed_fn
            self.input_ch = input_ch

        if multires_view > 0: # 视角的encodeing
        ...
        self.pts_linears = nn.ModuleList(
            ...
            [nn.Linear(W, W) if i not in self.skips else nn.Linear(W + self.input_ch, W) for i in range(D - 1)])  # 8层

        ### Implementation according to the official code release
        ### (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        self.views_linears = nn.ModuleList([nn.Linear(self.input_ch_view + W, W // 2)]) # decoder

        ### Implementation according to the paper
        # self.views_linears = nn.ModuleList(
        #     [nn.Linear(input_ch_views + W, W//2)] + [nn.Linear(W//2, W//2) for i in range(D//2)])

        if use_viewdirs: #使用viewdirs
            self.feature_linear = nn.Linear(W, W) 
            self.alpha_linear = nn.Linear(W, 1) # 分出alpha
            self.rgb_linear = nn.Linear(W // 2, 3) # 分出rgb，该层的分法是通过视角决定的
        else:
            self.output_linear = nn.Linear(W, output_ch) # 不用视角就直接输出

    def forward(self, input_pts, input_views): # 默认出alpha和rgb
        ...
        if self.use_viewdirs: # 使用视角来解码出最终的颜色和透明度和几何feature
    ...
    def density(self, input_pts): # 单出alpha
        if self.embed_fn is not None:
            input_pts = self.embed_fn(input_pts)
...
def IPE(mean, var, min_deg, max_deg): # 积分方向编码
...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    cdf = torch.cat([torch.zeros_like(cdf[..., :1]), cdf], -1) # [512,64]
...
class AppShadingNetwork(nn.Module): # 着色网络，主要预测各种用于渲染的参数
    default_cfg = {
        'human_light': False,
    ...
    def __init__(self, cfg):
        ...
        FG_LUT = torch.from_numpy(np.fromfile('assets/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2)) # FG_LUT是一个四维的张量（Tensor），它存储了一个叫做Fresnel-Schlick近似（Fresnel-Schlick approximation）的函数的查找表（LUT）。这个函数用于计算光线在不同材质表面的反射率https://filtergrade.com/free-cinematic-luts-video-editing/。FG_LUT是从一个二进制文件“assets/bsdf_256_256.bin”中读取的，这个文件包含了256x256个浮点数，每个浮点数表示一个反射率值https://lutify.me/free-luts/。FG_LUT的形状是(1, 256, 256, 2)，其中第一维是批次维（batch dimension），第二维和第三维是查找表的行和列，第四维是反射率的两个分量（R和G）
        self.register_buffer('FG_LUT', FG_LUT) # 缓冲区是一种特殊的属性，它不会被视为模块的参数（parameter），也不会在反向传播（backpropagation）时更新，但会随着模块一起保存和加载https://filmora.wondershare.com/video-editing/m31-lut.html。这样做的目的是为了在模块中保存一些固定的数据，比如查找表或者常数，以便在前向传播（forward pass）时使用

        self.sph_enc = generate_ide_fn(5) # 生成球面编码器
        self.dir_enc, dir_dim = get_embedder(6, 3) # 生成方向编码器
        self.pos_enc, pos_dim = get_embedder(self.cfg['light_pos_freq'], 3) # 光源位置编码器
    ...
    def predict_specular_lights(self, points, feature_vectors, reflective, roughness, human_poses, step): # 好复杂的感觉我天
    ...
    def predict_diffuse_lights(self, points, feature_vectors, normals):
        roughness = torch.ones([normals.shape[0], 1]) # ?roughness为1
        ref = self.sph_enc(normals, roughness)  # von Mises-Fisher distribution;是一种用于建模高维球面上随机变量的概率分布
        if self.cfg['sphere_direction']:
            sph_points = offset_points_to_sphere(points)
            ...
            light = self.outer_light(ref) # [58658,3]
        return light

    def forward(self, points, normals, view_dirs, feature_vectors, human_poses, inter_results=False, step=None):
        normals = F.normalize(normals, dim=-1)
        view_dirs = F.normalize(view_dirs, dim=-1)
        reflective = torch.sum(view_dirs * normals, -1, keepdim=True) * normals * 2 - view_dirs # 计算镜面出射方向
        ...
        # diffuse light
        diffuse_albedo = (1 - metallic) * albedo # 和作者所给公式基本一致;[58658,3]
        diffuse_light = self.predict_diffuse_lights(points, feature_vectors, normals) # L_diffuse;
        diffuse_color = diffuse_albedo * diffuse_light # C_diffuse

        # specular light
        specular_albedo = 0.04 * (1 - metallic) + metallic * albedo # (1-m)*0.04+m*a
        ...
        specular_ref = (specular_albedo * fg_lookup[:, 0:1] + fg_lookup[:, 1:2]) # specular_ref = specular_albedo * F1 +F2
